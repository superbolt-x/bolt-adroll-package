...
{%- set stg_fields = adapter.get_columns_in_relation(ref('_stg_adroll_campaigns_insights'))
                    |map(attribute="name")
...
WITH 
    ...
    (SELECT 
        ...
        {%- if field in currency_fields or '_revenue' in field %}
        ...
        {%- endfor %}
    FROM {{ ref('_stg_adroll_campaigns_insights') }}
    {%- if var('currency') != 'USD' %}
